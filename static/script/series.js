...
function updateGenres(page = 1) {
    ...
    fetch(`/series/{{ categoria }}?selected_genres=${selectedGenres.join(',')}&page=${page}`, {
        method: 'GET',
        headers: {
    ...
    .then(html => {
        document.querySelector('.series-grid').innerHTML = html;  // Actualiza solo la cuadrícula de series
